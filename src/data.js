import Facebook from '../src/assets/img/facebook.svg'
import Instagram from '../src/assets/img/instagram.svg'
import Twitter from '../src/assets/img/twitter.svg'
import Youtube from '../src/assets/img/youtube.svg'
import Linkedln from '../src/assets/img/linkedln.svg'

export const EXAMS_ITEMS = [
    {
      title: 'Banking intermediation',
      paragraf: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempo',
      img: '../assets/img/our_tran_img.png',
      label: '#1'
    },
    {
        title: 'Banking intermediation',
        paragraf: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempo',
        img: '../assets/img/our_tran_img.png',
        label: '#1'
      },
      {
        title: 'Banking intermediation',
        paragraf: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempo',
        img: '../assets/img/our_tran_img.png',
        label: '#1'
      },
    
  ];

export const Social = [
    {label:'/', icon: Instagram },
    {label:'/', icon: Twitter },
    {label:'/', icon: Youtube },
     {label:'/', icon: Linkedln },
     {label:'/', icon: Facebook }
];

export const FooterNav = [
    {label:'/Trainings', namenav:"Trainings"},
    {label:'/About', namenav:"About Aviza"},
    {label:'/', namenav:'FAQ'},
    {label:'/', namenav:'Exams'},
    {label:'/News', namenav:'News'},
    {label:'/Contact', namenav:'Contact'}
];

export const OUR_CARD = [
    {icon:' ', title:"Trust" , description:"Lorem ipsum dolor sit amet, consecteturadipiscing elit, sed do eiusmod tempo"},
    {icon:' ', title:"Trust" , description:"Lorem ipsum dolor sit amet, consecteturadipiscing elit, sed do eiusmod tempo"},
    {icon:' ', title:"Trust" , description:"Lorem ipsum dolor sit amet, consecteturadipiscing elit, sed do eiusmod tempo"},
    {icon:' ', title:"Trust" , description:"Lorem ipsum dolor sit amet, consecteturadipiscing elit, sed do eiusmod tempo"},
    {icon:' ', title:"Trust" , description:"Lorem ipsum dolor sit amet, consecteturadipiscing elit, sed do eiusmod tempo"}
];

export const CONTACT_CARD = [
    {icon:' ', title:"Phone" , description:"Koninklijkelaan 74 • 2600 Berchem"},
    {icon:' ', title:"Phone" , description:"+32 (0)3 346 47 48"},
    {icon:' ', title:"E-Mail" , description:"contact@avizaacademy.com"},
    
];

export const DISCOVER_ITEMS = [ 
  {
    title: 'Take Financial Training 1',
    paragraf: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempo2',
    img: '../assets/img/our_tran_img.png',
    label: '#1',
    id: 0
  },
  {
    title: 'Take Financial Training 2',
    paragraf: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempo2',
    img: '../assets/img/our_tran_img.png',
    label: '#2',
    id: 1
  },
  {
    title: 'Take Financial Training 3',
    paragraf: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempo3',
    img: '../assets/img/our_tran_img.png',
    label: '#3',
    id: 2
  },
  {
    title: 'Take Financial Training 4',
    paragraf: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempo3',
    img: "../assets/img/our_tran_img.png",
    label: '#4',
    id: 3
  },
  
];

export const NEWS_ITEMS = [ 
  {
    title: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit 1',
    paragraf: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempo',
    img: '../assets/img/our_tran_img.png',
    label: 'categories',
    id: 0
  },
  {
    title: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit 2',
    paragraf: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempo',
    img: '../assets/img/our_tran_img.png',
    label: 'categories',
    id: 1
  },
  {
    title: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit 3',
    paragraf: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempo',
    img: '../assets/img/our_tran_img.png',
    label: 'categories',
    id: 2
  },
  {
    title: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit 4',
    paragraf: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempo',
    img: "../assets/img/our_tran_img.png",
    label: 'categories',
    id: 3
  },
  
];

export const NEWS_DETAİL = [ 
  {
    title: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit',
    mainTitle: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit Main',
    paragraf: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. ',
    paragraf2: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. ',
    id : 0
  },
  {
    title: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit',
    paragraf: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. ',
    paragraf2: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. ',
    id : 1
  },
  {
    title: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit',
    paragraf: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. ',
    paragraf2: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. ',
    id : 2
  },
  
];

export const TRAINING_DATA = [ 
  {
    id: 1,
    info:'Workshop',
    title:'Practice Accounting - Sole proprietorships',
    paragraf: 'Do you want to get hands-on in bookkeeping and save several thousands of euros in the new year?',
    cards: [
      { cardText: 'Advanced' },
      { cardText: 'Classroom training'},
      { cardText: 'Accounting' },
    ],
    border: [
      { cardText: 'John Doe', redText: 'Level' },
      { cardText: '22 December 2022', redText: 'Level' },
      { cardText: 'FSMA Bank 27 punten', redText: 'Point' },
      { cardText: '€1206.37', redText: 'Price' },
    ],
  },
  {
    id: 2,
    info:'Workshop',
    title:'ESG & sustainable finance in investment funds',
    paragraf: 'Do you want to get hands-on in bookkeeping and save several thousands of euros in the new year?',
    cards: [
      { cardText: 'Bussines 2'},
      { cardText: 'Book Text 2'},
      { cardText: 'Paper 2'},
    ],
    border: [
      { cardText: 'John Doe', redText: 'Level' },
      { cardText: '22 December 2022', redText: 'Level' },
      { cardText: 'FSMA Bank 27 punten', redText: 'Point' },
      { cardText: '€1206.37', redText: 'Price' },
    ],
  },
  {
    id: 3,
    info:'Workshop',
    title:'ESG & sustainable finance in investment funds',
    paragraf: 'Do you want to get hands-on in bookkeeping and save several thousands of euros in the new year?',
    cards: [
      { cardText: 'Bussines 2'},
      { cardText: 'Book Text 2'},
      { cardText: 'Paper 2'},
    ],
    border: [
      { cardText: 'John Doe', redText: 'Level' },
      { cardText: '22 December 2022', redText: 'Level' },
      { cardText: 'FSMA Bank 27 punten', redText: 'Point' },
      { cardText: '€1206.37', redText: 'Price' },
    ],
  },
  {
    id: 4,
    info:'Workshop',
    title:'ESG & sustainable finance in investment funds',
    paragraf: 'Do you want to get hands-on in bookkeeping and save several thousands of euros in the new year?',
    cards: [
      { cardText: 'Bussines 2'},
      { cardText: 'Book Text 2'},
      { cardText: 'Paper 2'},
    ],
    border: [
      { cardText: 'John Doe', redText: 'Level' },
      { cardText: '22 December 2022', redText: 'Level' },
      { cardText: 'FSMA Bank 27 punten', redText: 'Point' },
      { cardText: '€1206.37', redText: 'Price' },
    ],
  },
  
  
  
];

export const EXAMPLES = [ 
  {
    title: 'About Training',
    description:
      'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.  ',
  },

  {
    title: 'About News',
    description:
    'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.  ',
  },

  {
    title: 'About',
    description:
    'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.  ',
  },  
 ];

 export const TRAM_DATA = [ 
  {
    title: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit',
    mainTitle: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit Main',
    paragraf: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. ',
    paragraf2: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. ',
    id : 0
  },
  {
    title: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit',
    paragraf: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. ',
    paragraf2: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. ',
    id : 1
  },
  {
    title: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit',
    paragraf: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. ',
    paragraf2: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. ',
    id : 2
  },
  {
    title: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit',
    paragraf: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. ',
    paragraf2: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. ',
    id : 2
  },
  {
    title: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit',
    paragraf: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. ',
    paragraf2: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. ',
    id : 2
  },
  
];







 
 
